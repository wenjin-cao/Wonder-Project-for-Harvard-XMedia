...
$lang = array_merge($lang, array(
	...
	'SYSTEM_TIMEZONE_EXPLAIN'         => '时区用来为未登录的用户（游客，爬虫）显示时区。新用户在注册过程中选择时区并且可以在登录后用户控制面板中修改。',
...
$lang = array_merge($lang, array(
	...
	'AVATAR_GALLERY_PATH_EXPLAIN'	=> '在您的 phpBB 论坛根目录下的预先设定的头像画册，例如 <samp>images/avatars/gallery</samp>',
	'AVATAR_STORAGE_PATH'			=> '头像存储路径',
	'AVATAR_STORAGE_PATH_EXPLAIN'	=> '在您的 phpBB 论坛根目录下的路径，例如 <samp>images/avatars/upload</samp>',
...
$lang = array_merge($lang, array(
	...
	'ALLOWED_SCHEMES_LINKS'				=> '允许在链接中有协议',
...
$lang = array_merge($lang, array(
	...
	'COOKIE_NAME_EXPLAIN'		=> '这可以是任何你希望的，确保原创。当你修改cookie设置时，这也应当被修改。',
	'COOKIE_NOTICE'				=> 'Cookie 提示',
	'COOKIE_NOTICE_EXPLAIN'		=> '启用 Cookie 提示后，用户会在访问论坛时得到提示。法律规定某些内容需要这个提示。',	
	'COOKIE_PATH'				=> 'Cookie 路径',
	'COOKIE_PATH_EXPLAIN'		=> '注意，这里总是有一个斜杠，无论你的论坛网址是怎样。',
...
// Contact Settings
$lang = array_merge($lang, array(
	'ACP_CONTACT_SETTINGS_EXPLAIN'		=> '这里您可以启用或禁用联系页也可以添加文本显示在联系页面。',
...
$lang = array_merge($lang, array(
	...
	'AUTH_PROVIDER_OAUTH_EXPLAIN'				=> '每个OAuth提供商需要唯一的密码和密钥，为了验证外部服务器。这些应该被OAuth服务提供，当您注册您的网站时应该输入准确。<br />任何没有密钥和密码的服务在这儿将不可用，也要注意用户仍可以注册和登录使用DB验证插件。',
	...
	'LDAP_EMAIL_EXPLAIN'			=> '将这个设置为用户的 email 属性名称(如果存在的话)，以便于为新用户自动设置email地址，留空的话将使第一次登录的用户email地址为空。',
	'LDAP_INCORRECT_USER_PASSWORD'	=> '使用指定的用户名密码绑定 LDAP服务器失败。',
...
// Server Settings
$lang = array_merge($lang, array(
	'ACP_SERVER_SETTINGS_EXPLAIN'	=> '这里配置服务器和域名相关的设定。请确保输入的数据是正确可靠的，错误将导致email包含错误信息，当输入域名时记住不包含 http:// 和其他协议头。只有当您的服务器使用一个特别的端口时才需要更改端口号，一般使用的都是80。',
	...
	'MOD_REWRITE_INFORMATION_UNAVAILABLE' => '我们不能决定是否此服务器支持路径重写，此设置可能启用但如果地址重写不可用，生成的路径(如链接)可能损坏。联系您的主机提供商如果您不确定是否您可以安全启用此功能。',
	'PATH_SETTINGS'				=> '路径设定',
	'RANKS_PATH'				=> '等级图标存储路径',
	...
	'USE_SYSTEM_CRON_EXPLAIN'		=> '关闭后 phpBB 将安排定期任务自动运行。启用后 phpBB 将不安排任何定期任务；一个系统管理员必须安排 <code>bin/phpbbcli.php cron:run</code> 运行间隔(例如每隔5分钟)。',
));
...
$lang = array_merge($lang, array(
	...
	'BROWSER_VALID'					=> '浏览器验证',
	'BROWSER_VALID_EXPLAIN'			=> '每次 Session 时，启用浏览器验证以增加安全性。',
	'CHECK_DNSBL'					=> '检查 IP 以防御 DNS 黑洞',
	'CHECK_DNSBL_EXPLAIN'			=> '如果启用，用户的 IP 地址将被检查以防御如下在注册和发帖时的 DNSBL 服务: <a href="http://spamcop.net">spamcop.net</a> 和 <a href="http://www.spamhaus.org">www.spamhaus.org</a>。这个检查将耗费一些时间，取决于服务器的设置。如果让论坛变得很慢或产生很多错误报告，请禁用这个功能。',
	'CLASS_B'						=> 'A.B',
	'CLASS_C'						=> 'A.B.C',
	'EMAIL_CHECK_MX'				=> '检查 email 域名以得到有效 MX 记录',
	'EMAIL_CHECK_MX_EXPLAIN'		=> '如果启用，在注册时提供的 email 的域名将被检查是否有有效的 MX 记录。',
	...
	'FORWARDED_FOR_VALID_EXPLAIN'	=> '只有在发送的 <var>X_FORWARDED_FOR</var> 字段头等于前一次请求中的字段头才继续会话。封禁也将检查 <var>X_FORWARDED_FOR</var> 中的 IP。',
	'IP_VALID'						=> 'Session IP 验证',
	'IP_VALID_EXPLAIN'				=> '决定用户的 IP 如何用于 Session 验证; <samp>所有</samp> 表示完整地址。<samp>A.B.C</samp> 表示开头的 x.x.x, <samp>A.B</samp> 表示开头的 x.x, <samp>None</samp> 取消验证.',
	'IP_LOGIN_LIMIT_MAX'			=> '每个 IP 的最大登陆尝试次数',
	'IP_LOGIN_LIMIT_MAX_EXPLAIN'	=> '单个 IP 登陆尝试时触发验证码机制的阈值次数。输入 0 防止验证码机制被 IP 触发。',
	'IP_LOGIN_LIMIT_TIME'			=> 'IP 登陆尝试过期时间',
...
$lang = array_merge($lang, array(
	...
	'ADMIN_EMAIL'					=> '返回 email 地址',
	'ADMIN_EMAIL_EXPLAIN'			=> '这将是所有 email 的返回地址，技术联络 email。将显示于<samp>Return-Path</samp> 和 <samp>Sender</samp>。',
	...
	'EMAIL_FUNCTION_NAME'			=> 'Email 函数名称',
	'EMAIL_FUNCTION_NAME_EXPLAIN'	=> '在 PHP 中用于发送 email 的函数。',
	'EMAIL_PACKAGE_SIZE'			=> 'Email 数据包大小',
	'EMAIL_PACKAGE_SIZE_EXPLAIN'	=> '这是在一个数据包中包含的最大 email 数量。这项设置被用于内部信件队列；如果您遇到无法投递信件的错误，请将它设置为 0。',
	'EMAIL_SIG'						=> 'Email 签名',
	...
	'SMTP_ALLOW_SELF_SIGNED_EXPLAIN'=> '允许连接到使用自签名 SSL 证书的 SMTP 服务器。 <em><strong>警告：</strong> 允许使用自签名的 SSL 证书可能带来安全隐患。</em>',
	...
	'SMTP_VERIFY_PEER_EXPLAIN'		=> '要求验证 SMTP 服务器所使用之 SSL 证书。 <em><strong>警告：</strong> 使用未验证之 SSL 证书的连接端会带来安全隐患。	</em>',
	'SMTP_VERIFY_PEER_NAME'			=> '验证 SMTP 端名字',
	'SMTP_VERIFY_PEER_NAME_EXPLAIN'	=> '要求验证SMTP 服务器的节点名字，并使用 SSL/TLS 连接。 <em><strong>警告：</strong> 连接到未验证的节点名字可能会导致安全隐患。</em>',
...
$lang = array_merge($lang, array(
	...
	'JAB_ALLOW_SELF_SIGNED_EXPLAIN'	=> '允许连接到使用自签名SSL证书的 Jabber 服务器。<em><strong>警告：</strong>允许自签名证书可能会带来安全隐患。 </em>',
	...
	'JAB_VERIFY_PEER_EXPLAIN'		=> '要求验证 Jabber 服务器的 SSL 证书。<em><strong>警告：</strong> 使用未验证的 SSL 证书来连接可能会带来安全隐患。 </em>',
	'JAB_VERIFY_PEER_NAME'			=> '验证 Jabber 节点名字',
	'JAB_VERIFY_PEER_NAME_EXPLAIN'	=> '要求验证使用 SSL/TLS 连接方式所连接的 Jabber 服务器的节点名字。<em><strong>警告：</strong> 连接到未验证的节点可能会带来安全隐患。</em>',
