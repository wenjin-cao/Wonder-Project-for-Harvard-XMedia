...
class helper 
{
	...
	public function __construct(driver_interface $db, config $config, manager $ext_manager, template $template, user $user, request $request, log $log, service $cache, $root_path)
	{
		$this->db			= $db;
		$this->config		= $config;
	...
	*/
	protected function version_check(\phpbb\extension\metadata_manager $md_manager, $force_update = false, $force_cache = false)
	{
		...
		$version_helper = (version_compare($this->config['version'], '1.0.1', '>')) ? 
	...
	*/
	public function update_files($replacements = array(), $revert)
	{
		...
		{
			$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, (($revert) ? 'LOG_BBCORE_DEINSTALLED' : 'LOG_BBCORE_INSTALLED'), time(), array());
		} else
		{
			$not_updated = array_diff($files, $files_changed);
			$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, (($revert) ? 'LOG_BBCORE_NOT_REPLACED' : 'LOG_BBCORE_NOT_UPDATED'), time(), array(implode('<br />', $not_updated)));
		}
	}
